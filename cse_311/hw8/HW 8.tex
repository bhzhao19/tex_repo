...
\begin{question}{Primal Translations [10 points]}
    ...
    \begin{enumerate}[label=(\alph*)]
        ...
        \begin{tcolorbox}
            ...
            $\exists y (\texttt{PRIME(x)}\land \texttt{PRIME(y)} \land (y-x = 4 \lor x-y = 4))$
        \end{tcolorbox}
        ...
        \begin{tcolorbox}
            ...
            $\forall N \exists x (\texttt{COUSIN-PRIME(x)} \land x \geq N)$
        \end{tcolorbox}
        ...
        \begin{align*}
            & \quad \;\; \neg \forall N \exists x (\texttt{COUSIN-PRIME(x)} \land x \geq N)  \\ & \equiv \exists N \forall x(\neg (\texttt{COUSIN-PRIME(x)} \land x \geq N)) 
                                                                                                && \text {Change quantifiers} \\
                                                                               & \equiv \exists N \forall x(\neg \texttt{COUSIN-PRIME(x)} \lor \neg (x \geq N)) 
                                                                                                && \text {DeMorgan's law} \\
                                                                               & \equiv \exists N \forall x(\neg \texttt{COUSIN-PRIME(x)} \lor x < N) 
                                                                                                && \text {Change inequality}\\ 
                                                                               & \equiv \exists N \forall x(\texttt{COUSIN-PRIME(x)} \longrightarrow x < N) 
                                                                                                && \text {Equivalence between or and implication}\\ 
        \end{align*}
        \end{tcolorbox} 
        \newpage
...
\begin{question}{Proof By Contradiction [11 points]}
    ...
    \begin{enumerate}[label=(\alph*)]
        ...
        \begin{tcolorbox}
            ...
            For the reason that $y$ can be written in the form of a fraction, where its numerator and denominator are all integers, by the definition of rational number, $y$ will be a 
